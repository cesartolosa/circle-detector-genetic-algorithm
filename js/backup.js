angular.module('circleDetectorApp', ['ui.bootstrap', 'ui.bootstrap-slider'])
    .controller("detectorController", function($scope) {
        ...
        Genotype.prototype.evaluate = function() {
        ...
        function evol(currentGeneration) {
            setTimeout(function() {
                ...
                if (currentGeneration > $scope.generations) {
                    ...
                    console.log('Best Genotype: ', getBestGenotype(population));
                    $scope.processing = false;
                    $scope.$apply();
                } else {
                    setPopulation(nextPopulation(population));
                    var bestGenotype = getBestGenotype(population);
                    console.log('Tamaño de la generación ' + currentGeneration + 'th: ' + population.length);
                    if ($scope.bestGenotypes.indexOf(bestGenotype) === -1) {
        ...
        function nextPopulation(basePopulation) {
            var bestGenotypes = [];
            var auxPopulation = _.sortBy(basePopulation, function(genotype) {
                return genotype.evaluate();
            });
            auxPopulation.splice(0, Math.floor(basePopulation.length / 2));
            bestGenotypes.push(auxPopulation[auxPopulation.length - 1]);
            auxPopulation = auxPopulation.concat(auxPopulation);
            bestGenotypes = crossOver(auxPopulation);
            basePopulation = _.sortBy(bestGenotypes, function(genotype) {
                return genotype.evaluate();
            });
            while (bestGenotypes.length > $scope.populationSize) {
                bestGenotypes.splice(0, 1);
            }
